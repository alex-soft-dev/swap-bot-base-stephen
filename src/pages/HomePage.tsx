...
function HomePage() {
    ...
    const selectWallet = async (id: number) => {
        ...
        await axios.get(`${process.env.REACT_APP_API_URL}/tradingInfos/getTradingStatus/${id}`, {headers})
        .then(function(response) {
            console.log(response.data)
            if(response.data.currentTrading === null) {
        ...
        .catch(function (error) {
            if(error.response.data.currentTrading === null) {
                toast.error(error.response.data.message);          
                // setSelectedWallet(JSON.parse(atob(error.response.data.selectedWallet)));
                setTokenAddress("")
                setBuyAmount(0)
                setSellAmount(0)
                setFrequency(0)
    ...
    const getTradingStatus = async () => {
        await axios.get(`${process.env.REACT_APP_API_URL}/tradingInfos/getLastTradingStatus`, {
            headers: {
    ...
    const stopTrading = async () => {
        ...
        await axios.post(`${process.env.REACT_APP_API_URL}/tradingInfos/stopTrading`, body, { headers })
            .then(function (response) {
                toast.success(response.data.message);
                    ...
                    <Row>
                        ...
                        <Col sm={12} lg={4} className="mb-5">
                            <MainCard title="Trading Info">
                                <Row>
                                    <Col xs={12} lg={7} className="mb-3 d-flex align-items-center">
                                        {walletList.length < 1 ? (
                                            <Button className="mb-3 Main-btn rounded-0 fw-bold" onClick={createWallet}>CREATE WALLET</Button>
                                        ) : (
                                            <Dropdown role="menuitemradio">
                                                <Dropdown.Toggle variant="success" id="dropdown-basic" className="Main-btn rounded-0 fw-bold mb-3">
                                        ...
                                        <Button className="mb-3 ms-3 text-center rounded-0 py-1 px-2 bg-transparent border-0" onClick={() => { copy(selectedWallet?.privateKey as string as string); toast.info("Private key Copied!"); console.log(base58.encode(Object.values(JSON.parse(selectedWallet?.privateKey as string)))) }}> </Button>
                                    </Col>
                                    <Col xs={12} lg={5} className="mb-3 text-start fw-bold fs-6 d-flex align-items-center justify-content-start justify-content-lg-end">
                        ...
                        <Col sm={12} lg={8} className="ms-auto mb-5">
